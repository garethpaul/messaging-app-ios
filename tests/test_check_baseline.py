...
class HomeTimeValidationContractTests(unittest.TestCase):
    ...
    def test_checker_rejects_makefile_skipping_tests(self):
        makefile = self.snapshot_root / "Makefile"
        command = '\tenv -i HOME="$(HOME)" PATH="/usr/local/bin:/opt/homebrew/bin:/usr/bin:/bin:/usr/sbin:/sbin" PYTHONDONTWRITEBYTECODE=1 python3 -I scripts/run-isolated-tests.py test\n'
    ...
    def test_checker_rejects_makefile_ignoring_test_failure(self):
        makefile = self.snapshot_root / "Makefile"
        command = "python3 -I scripts/run-isolated-tests.py test"
