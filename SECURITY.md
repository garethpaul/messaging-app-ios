...
- On GNU Make 4.3+, reject literal `$(` and `${` in repository Makefile paths
  before using `MAKEFILE_LIST` to derive trusted verification roots. GNU Make
  3.81 and 4.2 expand that path before repository rules run, so Make syntax in
  a checkout path is outside the enforceable local boundary on those versions.
