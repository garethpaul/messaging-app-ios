...
- On GNU Make 4.3+, repository Makefiles must reject literal `$(` and `${` path
  text before `MAKEFILE_LIST` is passed through shell-backed root resolution.
  GNU Make 3.81 and 4.2 expand such text before repository rules can inspect
  it, so validation must not overstate protection on those versions.
